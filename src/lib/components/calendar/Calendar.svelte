...
// Desktop state
let eventsPromise: Promise<CalendarEvent[]> = $state(Promise.resolve([]));
...
// Mobile state
let mobileMonths: { month: number, year: number, events: CalendarEvent[] }[] = $state([]);
...
function loadPrevWeek() {
	...
	eventsPromise = loadWeeks(newStart);
}
...
onMount(async () => {
	...
	if (isMobile) {
		...
		eventsPromise = loadWeeks(getStartOfWeek(new Date()));
	}
});
</script>
	...
	{:then events}
		...
		<div class="calendar-nav desktop-view">
			...
			<span class="calendar-month-label">
                Semaine du {weekStart.toLocaleDateString(undefined, { day: '2-digit', month: '2-digit', year: '2-digit' })}
			</span>
