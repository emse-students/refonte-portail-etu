...
import AssociationHandlePage from "../../../src/routes/associations/[handle]/+page.svelte";
...
describe("Associations Handle Page Component", () => {
	const mockAssociation = {
		id: 1,
		name: "Test Association",
		handle: "test-asso",
		description: "A test association description",
		members: [
			{
				id: 1,
				user: {
	...
	it("renders association name", () => {
		...
		expect(screen.getByRole("heading", { level: 1 })).toHaveTextContent("Test Association");
	});
	...
	it("shows empty state when no events", () => {
		...
		expect(screen.getByText("Aucun événement à venir pour cette association.")).toBeInTheDocument();
	});
	...
	it("shows edit association button when user has ADMIN permission on association", async () => {
		const adminMember = {
			id: 1,
			first_name: "Admin",
			last_name: "Member",
	...
	it("opens edit association modal", async () => {
		const adminUser = {
			id: 1,
			first_name: "Admin",
			last_name: "User",
		...
		expect(screen.getByLabelText("Nom de l'association")).toHaveValue("Test Association");
	});

	it("updates association details", async () => {
		const adminUser = {
			id: 1,
			first_name: "Admin",
			last_name: "User",
		...
		await fireEvent.input(nameInput, { target: { value: "Updated Association" } });
		...
		await waitFor(() => {
			expect(global.fetch).toHaveBeenCalledWith(
				`/api/associations/${mockAssociation.id}`,
				expect.objectContaining({
					method: "PUT",
					body: expect.stringContaining("Updated Association"),
				})
			);
		});
	});
